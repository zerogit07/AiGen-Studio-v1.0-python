...
def log_activity(user_id: int, model_id: str, prompt: str) -> None:
    ...
    global_stats.model_usage[model_id] = global_stats.model_usage.get(model_id, 0) + 1
    global_logs.insert(
        0,
        JobLog(user_id=user_id, model_id=model_id, timestamp=int(time.time() * 1000), prompt=prompt),
    )
